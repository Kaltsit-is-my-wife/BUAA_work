...
# 加载数据；注意：IMDB(split='train') 返回的是一次性可迭代对象，构建词表时会消耗
# 因此将其先转为列表，以便后续复用
...
# 3) 返回 [seq_len, batch]，并直接移动到 device
def collate_batch(batch):
    ...
    # pad_sequence 默认返回 [max_len, batch]（batch_first=False），与原 RNN 输入一致
    ...
    return SimpleNamespace(text=padded_text.to(device), label=labels.to(device))


# 与旧接口命名保持一致，便于复用训练/验证函数
...
def train(model, iterator, optimizer, criterion):
    ...
    for batch in iterator:
        optimizer.zero_grad()
        predictions = model(batch.text).squeeze(1)   # [batch]
...
def predict_sentiment(model, sentence: str):
    ...
    if ids.numel() == 0:
        ids = torch.tensor([vocab['<unk>']], dtype=torch.long)
    ids = ids.unsqueeze(1).to(device)  # [seq_len] -> [seq_len, 1]
    with torch.no_grad():
